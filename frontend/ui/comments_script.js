...
function ensureAuthOverlay() {
  ...
  authOverlay.innerHTML = `
    <div class="auth-card">
      <h2>Вход</h2>
      <p>Введите почту из справочника WikiLive.</p>
      <input type="email" id="authEmail" placeholder="Почта" />
      <input type="password" id="authPassword" placeholder="Пароль" />
      <div class="auth-error" id="authError"></div>
      <button type="button" id="authSubmit">Войти</button>
    </div>
  `;
  ...
  authSubmit.addEventListener("click", async () => {
    ...
    if (!email || !password) {
      authError.textContent = "Введите почту и пароль.";
      return;
    }
    ...
    try {
      ...
      if (response && response.user) {
        ...
        authError.textContent = "Неверная почта или пароль.";
      }
    } catch (error) {
      authError.textContent = "Не удалось войти. Проверьте сервер.";
...
let commentAutoVersionTimer = null;
let commentLastVersionAt = 0;
let commentLocalDraftTimer = null;
...
const COMMENT_AUTOVERSION_INTERVAL_MS = 10 * 60 * 1000;
...
function upsertCommentPage(page) {
  ...
  if (typeof renderBacklinksList === "function") {
    renderBacklinksList();
  }
}
...
function buildWikiLiveLinkSections() {
  const currentPage = {
    ...
    content: typeof window.wikiliveGetSerializableEditorHtml === "function"
...
function collectBacklinkPages() {
  if (!commentPageId) {
    return [];
  }
  const encodedId = encodeURIComponent(commentPageId);
  const token = `wikilive://page/${encodedId}`;
  return (commentPages || []).filter((page) => {
    if (!page || !page.pageId || page.pageId === commentPageId) {
      return false;
    }
...
function renderBacklinksList() {
  ...
  const backlinks = collectBacklinkPages();
  if (!backlinks.length) {
    backlinksList.innerHTML = '<div class="outline-sidebar__empty">Пока нет ссылок на эту страницу</div>';
    return;
  }
  backlinksList.innerHTML = backlinks.map((page) => `
    <button class="outline-sidebar__item outline-sidebar__item--backlink" type="button" data-backlink-page="${escapeHtml(page.pageId)}">
      <span class="outline-sidebar__item-badge">L</span>
      <span class="outline-sidebar__item-text">${escapeHtml(page.title || "Без названия")}</span>
    </button>
  `).join("");
}
...
async function saveCommentDocumentNow(label = "Изменение текста", author = "editor") {
  ...
  try {
    ...
    documentPayload.skipVersion = true;
...
function serializeEditorDocument() {
  ...
  return {
    ...
    content,
    mwsScenarioId: currentPage && currentPage.mwsScenarioId ? currentPage.mwsScenarioId : "",
    mwsScenarioTitle: currentPage && currentPage.mwsScenarioTitle ? currentPage.mwsScenarioTitle : ""
  };
}
...
function extractVersionSnapshotFromHtml(title, description, html) {
  ...
  root.querySelectorAll(".embedded-image-block").forEach((node) => {
    ...
    node.replaceWith(document.createTextNode(`[Изображение] ${label || "Изображение"}`));
  });
  root.querySelectorAll(".embedded-file-chip").forEach((node) => {
    const label = normalizeVersionLineText(node.textContent || "Объект");
    node.replaceWith(document.createTextNode(`[Объект] ${label || "Объект"}`));
  });
  root.querySelectorAll("[data-live-token], [data-live-value], .live-data-chip, .formula-chip, .table-value-chip").forEach((node) => {
    const label = normalizeVersionLineText(node.textContent || node.getAttribute("data-live-token") || node.getAttribute("data-live-value") || "Данные");
    node.replaceWith(document.createTextNode(`[Данные] ${label || "Данные"}`));
  });
  root.querySelectorAll("table").forEach((node) => {
    ...
    const label = rows || cols ? `[Таблица] ${rows}x${cols}` : "[Таблица]";
...
function inferDocumentVersionLabel(previousSnapshot, nextSnapshot, fallbackLabel = "Изменение текста") {
  if (!nextSnapshot || !previousSnapshot) {
    return fallbackLabel || "Изменение текста";
  }
  ...
  if (titleChanged && !descriptionChanged && !contentChanged) {
    return "Изменено название страницы";
  }
  if (!titleChanged && descriptionChanged && !contentChanged) {
...
function getLocalDraftKey(pageId) {
  return `wikilive-draft-${pageId || "unknown"}`;
}
...
function saveLocalDraft() {
  ...
  try {
    window.localStorage.setItem(getLocalDraftKey(commentPageId), JSON.stringify(snapshot));
  } catch (error) {
...
function scheduleLocalDraftSave() {
  if (commentLocalDraftTimer) {
    clearTimeout(commentLocalDraftTimer);
  }
  commentLocalDraftTimer = window.setTimeout(saveLocalDraft, 700);
}

function tryRestoreLocalDraft(page) {
  if (!page || !page.pageId) {
    return;
  }
  const raw = window.localStorage.getItem(getLocalDraftKey(page.pageId));
  if (!raw) {
    return;
  }
  try {
    ...
    const pageTime = new Date(page.updatedAt || 0).getTime();
    if (draftTime && draftTime > pageTime) {
      const approved = window.confirm("Найдена локальная версия. Восстановить черновик?");
      if (approved) {
        titleEditor.textContent = draft.title || page.title || "";
        bodyEditor.innerHTML = draft.content && draft.content.trim() ? draft.content : `<p class="body-placeholder">${emptyHint}</p>`;
        renderOutline();
      }
    }
  } catch (error) {
...
async function createCommentVersion(label, author = getCurrentCommentActor().id) {
  ...
  if (commentAutoVersionTimer) {
    clearTimeout(commentAutoVersionTimer);
    commentAutoVersionTimer = null;
  }
...
function scheduleCommentDocumentSave(label = "", author = "editor") {
  ...
  scheduleAutoVersion(label, author);
  if (commentSaveTimer) {
    clearTimeout(commentSaveTimer);
  }
...
function scheduleAutoVersion(label = "", author = "editor") {
  if (!commentPageId) {
    return;
  }
  if (commentAutoVersionTimer) {
  ...
  const now = Date.now();
  const nextAt = commentLastVersionAt ? commentLastVersionAt + COMMENT_AUTOVERSION_INTERVAL_MS : now + COMMENT_AUTOVERSION_INTERVAL_MS;
  const delay = Math.max(2000, nextAt - now);
  commentAutoVersionTimer = window.setTimeout(async () => {
    commentAutoVersionTimer = null;
    try {
      const baseLabel = commentPendingVersionLabel || label || "Изменение текста";
      await createCommentVersion(baseLabel || "Изменение текста", author || "editor");
    } catch (error) {
      console.warn("Failed to create auto version", error);
    }
  }, delay);
}
...
commentsComposerSend.addEventListener("click", () => {
  ...
  if (backlinksList) {
    backlinksList.addEventListener("click", (event) => {
      const item = event.target.closest("[data-backlink-page]");
      if (!item) {
        return;
      }
      const pageId = item.dataset.backlinkPage || "";
      if (pageId) {
        switchCommentPage(pageId, { persist: true }).catch((error) => console.warn("Failed to open backlink page", error));
      }
    });
  }
