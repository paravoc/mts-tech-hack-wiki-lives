...
.mws-live-cell__image-link:hover{
  background:#ffe8ee;
}

.mws-live-cell__image.is-broken img{
  display:none;
}
...
function renderMwsGrid(block, payload) {
  ...
  const href = attachment.url || attachment.resourceUrl || "";

  if (attachment.isImage) {
    return `<div
      class="mws-live-cell__attachment"
      data-mws-attachment-url="${safeAttr(href)}"
      data-mws-attachment-name="${safeAttr(label)}"
      data-mws-attachment-mime="${safeAttr(attachment.mimeType || "")}"
    >
      <a
        href="${safeAttr(href)}"
        target="_blank"
        rel="noopener noreferrer"
        class="mws-live-cell__image-link"
  ...
  >
    ...
    <span>${safeHtml(label)}</span>
                      ...
                      if (meta.isImage && (meta.resourceUrl || textValue)) {
  const imageUrl = meta.resourceUrl || textValue;
  return `<td class="mws-live-cell">
    <div class="mws-live-cell__image">
      <img
        src="${safeAttr(imageUrl)}"
        alt="${safeAttr(fieldName)}"
        loading="lazy"
        onerror="this.parentElement.classList.add('is-broken');"
      />
      <a
        href="${safeAttr(imageUrl)}"
        target="_blank"
        rel="noopener noreferrer"
        class="mws-live-cell__image-link"
      >Открыть изображение</a>
      <div class="mws-live-cell__caption">${safeHtml(textValue || fieldName)}</div>
    </div>
  </td>`;
}
