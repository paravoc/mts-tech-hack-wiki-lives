...
namespace {
    ...
    wikilive::utils::Expected<HttpResponse> performJsonPost(
        ...
        if (performCode != CURLE_OK) {
            return wikilive::utils::makeUnexpected(wikilive::utils::makeError(
                ErrorCode::AiApiError,
                curlErrorMessage("Request to MWS GPT", performCode, errorBuffer),
                502,
                true));
        }
    ...
    wikilive::utils::Expected<std::string> executeMwsGptJsonRequest(
        ...
        if (statusCode == 401 || statusCode == 403) {
            return wikilive::utils::makeUnexpected(wikilive::utils::makeError(
                ErrorCode::AiApiError,
                "MWS GPT rejected the request. Check AI_API_KEY.",
        ...
        if (statusCode >= 500) {
            return wikilive::utils::makeUnexpected(wikilive::utils::makeError(
                ErrorCode::AiApiError,
                "MWS GPT temporary server error: HTTP " + std::to_string(statusCode),
                static_cast<int>(statusCode),
                true));
        }
        ...
        if (statusCode >= 400) {
            return wikilive::utils::makeUnexpected(wikilive::utils::makeError(
                ErrorCode::AiApiError,
                "MWS GPT request failed: HTTP " + std::to_string(statusCode),
                static_cast<int>(statusCode),
                false));
        }
    ...
    wikilive::utils::Expected<nlohmann::json> parseJsonResponse(const std::string& responseBody) {
        try {
            ...
            return wikilive::utils::makeUnexpected(wikilive::utils::makeError(
                ...
                std::string("Failed to parse MWS GPT JSON response: ") + exception.what(),
                502,
                false));
        }
    }

    wikilive::utils::Expected<std::string> extractChatAssistantContent(const nlohmann::json& response) {
        if (!response.contains("choices") || !response["choices"].is_array() || response["choices"].empty()) {
            return wikilive::utils::makeUnexpected(wikilive::utils::makeError(
                wikilive::utils::ErrorCode::AiApiError,
                "MWS GPT response does not contain choices",
                502,
                false));
        }
        ...
        if (!first.is_object() || !first.contains("message") || !first["message"].is_object()) {
            return wikilive::utils::makeUnexpected(wikilive::utils::makeError(
                wikilive::utils::ErrorCode::AiApiError,
                "MWS GPT response does not contain message object",
                502,
                false));
        }
        ...
        if (!message.contains("content") || !message["content"].is_string()) {
            return wikilive::utils::makeUnexpected(wikilive::utils::makeError(
                wikilive::utils::ErrorCode::AiApiError,
                "MWS GPT response does not contain assistant content",
                502,
                false));
        }
    ...
    std::string buildCommentFixSystemPrompt() {
        return
            "Ты редактор комментариев в корпоративной wiki. "
            "Исправь орфографию, пунктуацию и явные грамматические ошибки. "
            "Не меняй смысл, стиль и тон автора. "
            "Не добавляй новых фактов. "
            "Верни только исправленный текст без пояснений и без кавычек.";
    }

    std::string buildCommentFixUserPrompt(const std::string& text) {
        return "Исправь комментарий:\n\n" + text;
    }
...
namespace wikilive::server {
    ...
    Router::Router(
        ...
        aiBaseUrl_(std::move(aiBaseUrl)),
        aiApiKey_(std::move(aiApiKey)),
        aiModel_(std::move(aiModel)) {
    }
    ...
    RouteResponse Router::replyToComment(
        ...
        try {
            ...
            const auto actorToken = parsedPayload.value("author", std::string("viewer"));
            if (page->commentAccessMode == "owner_only" && actorToken != page->ownerId) {
                return fail(utils::makeError(
                    utils::ErrorCode::InvalidRequest,
                    "Access denied for comments",
                    403,
                    false));
            }
    ...
    RouteResponse Router::fixCommentText(const std::string& payload) {
        try {
            if (payload.empty()) {
                return fail(utils::makeError(
                    utils::ErrorCode::InvalidRequest,
                    "AI comment fix payload must not be empty",
                    400,
                    false));
            }

            const auto parsedPayload = nlohmann::json::parse(payload, nullptr, true, true);
            const auto text = parsedPayload.value("text", std::string{});

            if (text.empty()) {
                return fail(utils::makeError(
                    utils::ErrorCode::InvalidRequest,
                    "AI field text must not be empty",
                    400,
                    false));
            }
            ...
            if (text.size() > 4000) {
                return fail(utils::makeError(
                    utils::ErrorCode::InvalidRequest,
                    "AI field text is too long",
                    400,
                    false));
            }

            const std::string baseUrl = aiBaseUrl_.empty()
                ? "https://api.gpt.mws.ru/v1"
                : aiBaseUrl_;
            const std::string apiKey = aiApiKey_;
            const std::string model = aiModel_;
            ...
            const auto parsedBaseUrl = parseBaseUrl(baseUrl);
            if (!parsedBaseUrl) {
                return fail(parsedBaseUrl.error());
            }
            ...
            nlohmann::json requestBody = {
                ...
                {"messages", nlohmann::json::array({
                    {
                        {"role", "system"},
                        {"content", buildCommentFixSystemPrompt()},
                    },
                    {
                        {"role", "user"},
                        {"content", buildCommentFixUserPrompt(text)},
                    },
                })},
                {"temperature", 0.1},
                {"max_tokens", 300},
            ...
            const auto response = executeMwsGptJsonRequest(
                parsedBaseUrl.value(),
                "/chat/completions",
                apiKey,
                10000,
            ...
            const auto parsedResponse = parseJsonResponse(response.value());
            if (!parsedResponse) {
                return fail(parsedResponse.error());
            }

            const auto fixedText = extractChatAssistantContent(parsedResponse.value());
            if (!fixedText) {
                return fail(fixedText.error());
            }
            ...
            return fail(utils::makeError(
                utils::ErrorCode::InvalidRequest,
                std::string("Malformed AI comment fix JSON payload: ") + exception.what(),
                400,
                false));
        }
        catch (const std::exception& exception) {
    ...
    utils::Expected<services::PageDraft> Router::parsePagePayload(const std::string& payload) const {
        ...
        if (!title) {
            return wikilive::utils::makeUnexpected(title.error());
        }

        auto projectId = extractJsonString(payload, "projectId", false);
        if (!projectId) {
            return wikilive::utils::makeUnexpected(projectId.error());
        }
        ...
        if (!description) {
            return wikilive::utils::makeUnexpected(description.error());
        }

        auto ownerId = extractJsonString(payload, "ownerId", false);
        if (!ownerId) {
            return wikilive::utils::makeUnexpected(ownerId.error());
        }

        auto ownerName = extractJsonString(payload, "ownerName", false);
        if (!ownerName) {
            return wikilive::utils::makeUnexpected(ownerName.error());
        }
