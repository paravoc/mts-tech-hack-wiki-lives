...
wikilive::utils::Expected<std::string> executeMwsGptJsonRequest(
    ...
    if (!connection) {
        return std::unexpected(wikilive::utils::makeError(
            ErrorCode::AiApiError,
            lastWinHttpErrorMessage("WinHTTP connection to MWS GPT"),
            502,
            true));
    }
    ...
    if (!request) {
        return std::unexpected(wikilive::utils::makeError(
            ErrorCode::AiApiError,
            lastWinHttpErrorMessage("WinHTTP MWS GPT request initialization"),
            502,
            true));
    }
    ...
    if (!WinHttpSendRequest(
        ...
        return std::unexpected(wikilive::utils::makeError(
            ErrorCode::AiApiError,
            lastWinHttpErrorMessage("Sending request to MWS GPT"),
            502,
            true));
    }
    ...
    if (!WinHttpReceiveResponse(request.get(), nullptr)) {
        return std::unexpected(wikilive::utils::makeError(
            ErrorCode::AiApiError,
            lastWinHttpErrorMessage("Receiving response from MWS GPT"),
            502,
            true));
    }
    ...
    if (!WinHttpQueryHeaders(
        ...
        return std::unexpected(wikilive::utils::makeError(
            ErrorCode::AiApiError,
            lastWinHttpErrorMessage("Reading MWS GPT HTTP status code"),
            502,
            true));
    }
    ...
    for (;;) {
        ...
        if (!WinHttpQueryDataAvailable(request.get(), &availableSize)) {
            return std::unexpected(wikilive::utils::makeError(
                ErrorCode::AiApiError,
                lastWinHttpErrorMessage("Querying MWS GPT response size"),
                502,
                true));
        }
        ...
        if (!WinHttpReadData(request.get(), buffer.data(), availableSize, &bytesRead)) {
            return std::unexpected(wikilive::utils::makeError(
                ErrorCode::AiApiError,
                lastWinHttpErrorMessage("Reading MWS GPT response body"),
                502,
                true));
        }
